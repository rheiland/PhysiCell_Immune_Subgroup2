...
std::vector<double> choose_vascularized_position( void )
{
	...
	//std::cout<<"vascular position"<<std::endl;
	...
	//std::cout<<"left "<<microenvironment.mesh.voxels[n].center<<std::endl;
...
{
	//std::cout<<"infiltrating neut"<<std::endl;
...
{
	//std::cout<<"infiltrating CD8T"<<std::endl;
...
{
	//std::cout<<"infiltrating CD4T"<<std::endl;
...
{
	//std::cout<<"infiltrating DC"<<std::endl;
...
void create_infiltrating_macrophage(void)
{
	...
	//std::cout<<"infiltrating mac"<<std::endl;
...
void CD8_Tcell_contact_function( Cell* pC1, Phenotype& p1, Cell* pC2, Phenotype& p2 , double dt )
{
	...
	//std::cout<<"CD8 T cell contact"<<std::endl;
...
void CD8_Tcell_mechanics( Cell* pCell, Phenotype& phenotype, double dt )
{
	...
	//std::cout<<"CD8 mechanics"<<std::endl;
	...
	{
		//std::cout<<"adhered "<<pCell->state.number_of_attached_cells()<<std::endl;
		...
		if( detach_me )
		{
			...
			// check for cells to kill
			//std::vector<Cell*> neighbors = pCell->cells_in_my_container(); 
			//int n = 0; 
			//Cell* pCX = neighbors[n]; 
			//while( n < neighbors.size() )
			//{
				//pCX = neighbors[n]; 
				//if(pCX->phenotype.molecular.internalized_total_substrates[nV_external]>1e-6 && 
				//pCX->type == epithelial_type) // cell is infected lung tissue
				//{
					//#pragma omp critical
					//{
					//	pCX->die();
					//	std::cout<<"CD8 Killed infected cell"<<std::endl;
					//}
					//n = neighbors.size();
				//}
				//n++;
...
//	std::cout<<"immune cell motility"<<std::endl;
...
{
	//std::cout<<"mac phenotype"<<std::endl;
	
	static int apoptosis_index = phenotype.death.find_death_model_index( "apoptosis" ); 
	static Cell_Definition* pCD = find_cell_definition( "macrophage" ); 
	static Cell_Definition* pCD_DC = find_cell_definition( "neutrophil" ); 
	...
	static int DCcell_type = get_cell_definition( "DC" ).type; 
	...
	while( n < neighbors.size() )
	{
		...
		phenotype.death.rates[apoptosis_index] = pCell->custom_data["exhausted_macrophage_death_rate"];
			...
			//std::cout<<"Mac in contact with CD8 so stops secreting: "<<pCell->phenotype.secretion.secretion_rates[proinflammatory_cytokine_index]<<std::endl;
		}
...
{
	//std::cout<<"mac mechanics"<<std::endl;
	static int debris_index = microenvironment.find_density_index( "debris");
...
{
	//std::cout<<"neut phenotype"<<std::endl;
	...
	// at least one of the cells is pCell, so don't do anything as it has no neighbours
	if( neighbors.size() < 2 )
	{ return; } 
	...
	double probability_of_phagocytosis = pCell->custom_data["phagocytosis_rate"] * dt; 
	double max_phagocytosis_volume = pCell->custom_data["threshold_macrophage_volume"];//pCell->custom_data["phagocytosis_relative_target_cutoff_size" ] * pCD->phenotype.volume.total; 
	...
	while( n < neighbors.size() )
	{
			...
			pTestCell->custom_data["Vnuc"]>parameters.doubles("Infection_detection_threshold")/Vvoxel )
		{
			...
			// (Adrianne) Cell starts secreting ROS
...
void CD4_Tcell_mechanics( Cell* pCell, Phenotype& phenotype, double dt )
{
	...
	//std::cout<<"CD4 T cell mechanics"<<std::endl;
...
void immune_submodels_setup( void )
{
	...
	//std::cout<<"immune submodels setup"<<std::endl;
...
Cell* check_for_live_neighbor_for_interaction( Cell* pAttacker , double dt )
{
	...
	//std::cout<<"check for live neighbour"<<std::endl;
...
Cell* check_for_dead_neighbor_for_interaction( Cell* pAttacker , double dt )
{
	...
	//std::cout<<"check for dead neighbour"<<std::endl;
	std::vector<Cell*> nearby = pAttacker->cells_in_my_container(); 
	int i = 0; 
	while( i < nearby.size() )
	{
...
void immune_cell_recruitment( double dt )
{
	...
	if( t_immune > t_next_immune- tolerance )
	{
		...
		//std::cout<<number_of_new_cells_int<<std::endl;
		//STOCHASTIC PORTION		
...
void initial_immune_cell_placement( void )
{
	...
	//std::cout<<"initial immune cell placement"<<std::endl;
...
void keep_immune_cells_off_edge( void )
{
	...
	//std::cout<<"keep off edge"<<std::endl;
...
void keep_immune_cells_in_bounds( double dt )
{
	...
	//std::cout<<"immune cells in bounds"<<std::endl;
